...
export default function Assessment() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (assessmentResult?.result) setResult(assessmentResult.result);
      } catch (e) {
  ...
  const remaining = assessment?.remainingAttempts ?? assessment?.attemptsRemaining;

  const submit = async () => {
    if (submitting) return;
    if (answered !== questions.length) { setError(`Please answer all questions. ${questions.length - answered} remaining.`); return; }
    ...
    } catch (e) { setError(e?.response?.data?.message || e?.message || "Unable to submit assessment."); }
    ...
    <Box sx={{ maxWidth: 900, mx: "auto" }}>
      ...
      <Card elevation={0} sx={{ border: "1px solid #e5e7eb", borderRadius: 4, overflow: "hidden", boxShadow: "0 18px 50px rgba(15,23,42,.07)" }}>
        ...
        <CardContent sx={{ p: { xs: 2.5, md: 4 } }}><Stack spacing={3}>
          ...
          {result ? <Box sx={{ p: { xs: 3, md: 5 }, textAlign: "center", borderRadius: 4, bgcolor: result.passed ? "#f0fdf4" : "#fff7ed", border: `1px solid ${result.passed ? "#bbf7d0" : "#fed7aa"}` }}><Box sx={{ width: 70, height: 70, mx: "auto", mb: 2, borderRadius: "50%", display: "grid", placeItems: "center", bgcolor: result.passed ? "#dcfce7" : "#ffedd5", color: result.passed ? "#16a34a" : "#ea580c" }}>{result.passed ? <EmojiEvents sx={{ fontSize: 40 }} /> : <Replay sx={{ fontSize: 38 }} />}</Box><Typography variant="h4" fontWeight={900}>{result.passed ? "Assessment Passed" : "Assessment Not Passed"}</Typography><Typography sx={{ mt: 1 }} color="text.secondary">Your score</Typography><Typography variant="h2" fontWeight={950}>{Number(result.score || 0).toFixed(1)}%</Typography><Chip sx={{ mt: 1 }} color={result.passed ? "success" : "warning"} label={result.passed ? `Passed • ${result.passingScore ?? assessment?.passingScore ?? 60}% required` : `Need ${result.passingScore ?? assessment?.passingScore ?? 60}% to pass`} />{result.passed && <Stack alignItems="center" spacing={1} sx={{ mt: 3 }}><CheckCircle color="success" /><Typography color="text.secondary">You can now continue to the certificate step.</Typography></Stack>}</Box> : questions.map((q, index) => <Box key={q._id || index} sx={{ p: 3, border: "1px solid #e5e7eb", borderRadius: 3 }}><Typography fontWeight={850}>Q{index + 1}. {q.question}</Typography><RadioGroup value={answers[q._id] || ""} onChange={(e) => setAnswers((a) => ({ ...a, [q._id]: e.target.value }))} sx={{ mt: 1 }}>{(q.options || []).map((o) => <FormControlLabel key={o._id} value={o._id} control={<Radio />} label={o.text} sx={{ alignItems: "flex-start" }} />)}</RadioGroup></Box>)}
          {!result && <><Divider /><Stack direction={{ xs: "column", sm: "row" }} justifyContent="space-between" alignItems={{ xs: "stretch", sm: "center" }} spacing={2}><Typography variant="body2" color="text.secondary">Answered {answered} of {questions.length}</Typography><Button variant="contained" onClick={submit} disabled={submitting || !questions.length} sx={{ minHeight: 48, px: 4, borderRadius: 2.5, fontWeight: 850, textTransform: "none" }}>{submitting ? "Submitting..." : "Submit Assessment"}</Button></Stack></>}
        </Stack></CardContent>
      </Card>
    </Box>
  </Box>;
