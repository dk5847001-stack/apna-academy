...
export default function CoursePlayerLayout({
  ...
  const handlePreviousClick = () => {
    if (previousVideo) onPrevious?.(previousVideo);
  };

  const handleNextClick = () => {
    if (nextVideo) onNext?.(nextVideo);
  };
    ...
    <Box sx={{ width: mobile ? "min(390px, 90vw)" : 370, height: "100%", display: "flex", flexDirection: "column", backgroundColor: "#ffffff" }}>
      <Box sx={{ px: 2.5, py: 2.5, borderBottom: "1px solid #e5e7eb" }}>
        <Stack direction="row" alignItems="center" justifyContent="space-between" spacing={2}>
          <Button variant="text" startIcon={<ArrowLeft />} onClick={onBack} sx={{ textTransform: "none", fontWeight: 700, color: "#334155", px: 0, "&:hover": { backgroundColor: "transparent", color: "#2563eb" } }}>
            Back to course
          </Button>
          {mobile && <IconButton onClick={() => setSidebarOpen(false)} aria-label="Close course menu"><X /></IconButton>}
        </Stack>

        <Typography sx={{ mt: 2, fontSize: { xs: "1.05rem", sm: "1.15rem" }, fontWeight: 800, lineHeight: 1.35, color: "#0f172a" }}>
        ...
        <Box sx={{ mt: 2.25 }}>
          <Stack direction="row" alignItems="center" justifyContent="space-between" sx={{ mb: 0.8 }}>
            ...
            <Typography variant="caption" fontWeight={800} color="primary.main">{Math.round(normalizedProgress)}%</Typography>
          </Stack>
      ...
      <Box sx={{ flex: 1, overflowY: "auto" }}>
        {safeModules.length === 0 ? (
          ...
          return (
            <Box key={module?._id || moduleKey} sx={{ borderBottom: "1px solid #e5e7eb" }}>
              <Button fullWidth onClick={() => setOpenModule(isOpen ? null : moduleKey)} sx={{ px: 2.5, py: 2, justifyContent: "space-between", textAlign: "left", textTransform: "none", color: "#0f172a", backgroundColor: isOpen ? "#eff6ff" : "#ffffff", borderRadius: 0, "&:hover": { backgroundColor: "#f8fafc" } }}>
                <Box sx={{ minWidth: 0, pr: 1 }}>
                  <Typography sx={{ fontSize: "0.92rem", fontWeight: 800, lineHeight: 1.45 }}>{moduleIndex + 1}. {module?.title || "Module"}</Typography>
                  <Typography variant="caption" color="text.secondary">{moduleVideos.length} lesson{moduleVideos.length === 1 ? "" : "s"}</Typography>
                </Box>
                <ExpandMore sx={{ flexShrink: 0, transform: isOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 180ms ease", color: isOpen ? "#2563eb" : "#64748b" }} />
              </Button>
              ...
              {isOpen && moduleVideos.length > 0 && (
                <Box sx={{ backgroundColor: "#f8fafc" }}>
                  {moduleVideos.map((video) => {
                    ...
                    return (
                      <Button key={getVideoId(video)} fullWidth disabled={locked} onClick={() => handleVideoClick(video)} sx={{ minHeight: 68, px: 2.5, py: 1.25, justifyContent: "flex-start", alignItems: "flex-start", gap: 1.25, textAlign: "left", textTransform: "none", borderRadius: 0, borderLeft: active ? "3px solid #2563eb" : "3px solid transparent", backgroundColor: active ? "#dbeafe" : "transparent", color: locked ? "#94a3b8" : "#334155", "&:hover": { backgroundColor: locked ? "transparent" : "#eef2ff" } }}>
                        <Box sx={{ width: 30, height: 30, mt: 0.1, flexShrink: 0, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: completed ? "#dcfce7" : locked ? "#e2e8f0" : active ? "#dbeafe" : "#ffffff", border: "1px solid", borderColor: completed ? "#bbf7d0" : active ? "#bfdbfe" : "#e2e8f0" }}>
                          {completed ? <Check sx={{ fontSize: 17, color: "#16a34a" }} /> : locked ? <Lock sx={{ fontSize: 16, color: "#64748b" }} /> : <PlayCircle sx={{ fontSize: 18, color: active ? "#2563eb" : "#64748b" }} />}
                        </Box>
                        <Box sx={{ minWidth: 0, flex: 1 }}>
                          <Typography sx={{ fontSize: "0.86rem", fontWeight: active ? 800 : 600, lineHeight: 1.4, color: locked ? "#94a3b8" : active ? "#1d4ed8" : "#334155" }}>{getVideoTitle(video)}</Typography>
                          <Stack direction="row" spacing={0.75} alignItems="center" sx={{ mt: 0.4 }}>
                            {video?.isPreview && <Chip label="Preview" size="small" sx={{ height: 20, fontSize: "0.65rem", fontWeight: 800, backgroundColor: "#eff6ff", color: "#2563eb" }} />}
                            {getVideoDuration(video) && <Typography variant="caption" color="text.secondary">{getVideoDuration(video)}</Typography>}
                          </Stack>
                        </Box>
                      </Button>
                    );
  ...
  return (
    <Box sx={{ minHeight: "100vh", backgroundColor: "#f8fafc", color: "#0f172a" }}>
      <Box sx={{ display: { xs: "flex", lg: "none" }, height: 64, alignItems: "center", justifyContent: "space-between", px: 1.5, backgroundColor: "#ffffff", borderBottom: "1px solid #e5e7eb", position: "sticky", top: 0, zIndex: 20 }}>
        <IconButton onClick={() => setSidebarOpen(true)} aria-label="Open course menu"><Menu /></IconButton>
        <Typography sx={{ maxWidth: "65%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontWeight: 800, fontSize: "0.95rem" }}>{courseTitle || course?.title || "Course"}</Typography>
        <Box sx={{ width: 40 }} />
      </Box>

      <Box sx={{ display: "flex", minHeight: { xs: "calc(100vh - 64px)", lg: "100vh" } }}>
        <Box component="aside" sx={{ display: { xs: "none", lg: "block" }, width: 370, flexShrink: 0, borderRight: "1px solid #e5e7eb", backgroundColor: "#ffffff" }}>
          {renderSidebarContent(false)}
        </Box>

        <Box component="main" sx={{ minWidth: 0, flex: 1 }}>
          <Box sx={{ p: { xs: 1.25, sm: 2.5, lg: 3.5 }, maxWidth: 1280, mx: "auto" }}>
            <Paper elevation={0} sx={{ overflow: "hidden", border: "1px solid #e5e7eb", borderRadius: { xs: 2, md: 3 }, backgroundColor: "#ffffff" }}>
              ...
              <Box sx={{ p: { xs: 2, sm: 2.5, md: 3 } }}>
                ...
                <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5} justifyContent="space-between">
                  <Button variant="outlined" startIcon={<ChevronLeft />} onClick={handlePreviousClick} disabled={!previousVideo} sx={{ textTransform: "none", fontWeight: 800, minHeight: 44, width: { xs: "100%", sm: "auto" } }}>
                    Previous
                  </Button>
                  <Button variant="contained" endIcon={<ChevronRight />} onClick={handleNextClick} disabled={!nextVideo} sx={{ textTransform: "none", fontWeight: 800, minHeight: 44, width: { xs: "100%", sm: "auto" } }}>
                    Next lesson
                  </Button>
                </Stack>
              </Box>
