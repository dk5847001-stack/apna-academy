...
export default function InterviewHistoryPage() {
  ...
  return <main className="history-page"><section className="history-shell">
    ...
    <section className="history-trend-card"><div><span className="result-eyebrow"><TrendingUpRounded /> SCORE TREND</span><strong>{history.length ? 'Recent session scores' : 'No completed sessions yet'}</strong></div>{history.length ? <div className="history-trend-bars">{[...history].slice(0, 8).reverse().map((item) => <div className="history-trend-item" key={item.id} title={item.role + ': ' + item.score + '/100'}><span style={{ height: Math.max(6, item.score) + '%' }} /><small>{item.score}</small></div>)}</div> : null}</section>
