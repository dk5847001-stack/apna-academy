  ...
  return <main className="result-page">
    ...
    <section className="result-shell">
      ...
      <section className="result-overview">
        <div className="score-card"><span className="score-label">OVERALL SCORE</span><div className="score-ring"><div><strong>{result.overallScore}</strong><small>/100</small></div></div><div className="score-grade"><EmojiEventsRounded /> {result.grade} performance</div><p>Top {100-result.percentile}% of your recent practice sessions</p></div>
        <div className="result-metrics"><div className="metrics-head"><div><span>Performance breakdown</span><small>AI-generated estimates based on this practice session</small></div><InsightsRounded /></div>{result.metrics.map((metric) => <div className="metric-row" key={metric.key}><div className="metric-title"><strong>{metric.label}</strong><span>{metric.score}%</span></div><div className="metric-track"><i style={{width: metric.score+'%'}} /></div><small>{metric.note}</small></div>)}</div>
        <div className="session-card"><span>SESSION</span><strong>{role}</strong><div><small>Questions</small><b>{result.total}</b></div><div><small>Answered</small><b>{session?.answers?.length || result.answered}</b></div><div><small>Duration</small><b>{result.duration}</b></div></div>
      </section>
      <section className="result-two-col">
        <div className="insight-panel"><div className="panel-heading"><span className="panel-icon positive"><TrendingUpRounded /></span><div><h2>What you did well</h2><p>Strengths to carry into your next interview.</p></div></div><div className="insight-list">{result.strengths.map((item) => <div key={item}><CheckCircleRounded /><span>{item}</span></div>)}</div></div>
        <div className="insight-panel"><div className="panel-heading"><span className="panel-icon improve"><WarningAmberRounded /></span><div><h2>Where to improve</h2><p>Small changes that can make your answers stronger.</p></div></div><div className="insight-list">{result.improvements.map((item) => <div key={item}><WarningAmberRounded /><span>{item}</span></div>)}</div></div>
      </section>
      <section className="recommend-section"><div className="recommend-head"><div><span>PERSONALIZED NEXT STEPS</span><h2>Keep building your edge.</h2></div><AutoAwesomeRounded /></div><div className="recommend-grid">{result.recommendations.map((item) => <button className="recommend-card" key={item.title} onClick={() => navigate(ROUTES.INTERVIEW_SETUP)}><span>{item.type}</span><strong>{item.title}</strong><small>{item.meta}</small><ArrowForwardRounded /></button>)}</div></section>
    </section>
  </main>
}
