  ...
  const loadProfile = async () => {
    ...
    try {
      ...
      try {
        ...
        setCertificates(certificateData?.certificates || certificateData || []);
      } catch {
        setCertificates([]);
      }
    } catch (err) {
      setError(err?.response?.data?.message || err?.message || "Unable to load your profile.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <div className="mb-7">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
          Manage your student information and review your account status.
        </p>
      </div>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
            <Avatar
              ...
              sx={{ width: 84, height: 84, fontSize: 25, fontWeight: 800, bgcolor: "#dbeafe", color: "#1d4ed8" }}
            >
              {initials}
            </Avatar>
          ...
          <form onSubmit={handleSave} className="space-y-5">
            ...
            <div className="flex justify-end pt-1">
              <Button type="submit" variant="contained" disabled={saving} sx={{ borderRadius: "12px", px: 3, py: 1.25, textTransform: "none", fontWeight: 800 }}>
        ...
        <div className="space-y-5">
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-extrabold text-slate-900">Email verification</h3>
                <p className="text-xs text-slate-500">Your login email</p>
              </div>
            </div>
            <div className="mt-5 flex items-center justify-between gap-3 rounded-2xl bg-slate-50 p-3">
              <span className="min-w-0 truncate text-xs font-semibold text-slate-600">{displayUser.email || "—"}</span>
              <Chip size="small" label={displayUser.isEmailVerified ? "Verified" : "Unverified"} color={displayUser.isEmailVerified ? "success" : "warning"} variant="outlined" />
            </div>
          </section>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-extrabold text-slate-900">Certificates</h3>
                <p className="text-xs text-slate-500">Issued to your account</p>
              </div>
            </div>
          ...
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Protected authentication session</p>
              </div>
            </div>
      ...
      <section className="mt-5 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">
        <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center">
          <div>
            <h2 className="text-base font-extrabold text-slate-900">Learning overview</h2>
            <p className="mt-1 text-sm text-slate-500">Your enrolled courses are managed from My Courses.</p>
          </div>
          <Chip label={`${enrolledCourses.length} enrolled`} variant="outlined" color="primary" />
        </div>
      </section>
