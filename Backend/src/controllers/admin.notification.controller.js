...
export const updateNotification = asyncHandler(async (req, res) => {
  const data = await updateAdminNotification({ notificationId: req.params.notificationId, userId: req.body?.userId, title: req.body?.title, message: req.body?.message, type: req.body?.type, link: req.body?.link });
  return successResponse({ res, message: "Notification updated successfully.", data });
});
