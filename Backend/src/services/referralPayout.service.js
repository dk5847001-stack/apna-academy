...
export const requestReferralWithdrawal=async({userId,...input})=>{
 const d=normalize(input),s=await ReferralWallet.startSession();
 try{let out;await s.withTransaction(async()=>{
  ...
  const now=new Date();
  const payout=(await ReferralPayout.create([{user:userId,amountPaise:amount,currency:"INR",status:"requested",payoutMethod:d.method,destinationSnapshot:{method:d.method,accountHolderName:d.accountHolderName,...(d.method==="upi"?{upiId:d.upiId}:{accountNumberLast4:d.accountNumberLast4,ifsc:d.ifsc})},requestedAt:now,idempotencyKey:"withdrawal:"+userId+":"+crypto.randomUUID()}],{session:s}))[0];
