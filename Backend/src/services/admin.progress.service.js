...
const formatProgress = (item) => ({
  ...
  completedVideoCount: Array.isArray(item.completedVideos) ? item.completedVideos.length : 0,
  ...
  lastWatchedVideo: item.lastWatchedVideo ? { id: item.lastWatchedVideo._id?.toString(), title: item.lastWatchedVideo.title } : null,
});
...
export const listAdminProgress = async ({ page = 1, limit = 20, search = "", courseId = "", completed = "" } = {}) => {
  ...
  if (search?.trim()) {
    ...
    userIds = await User.find({ $or: [{ name: { $regex: regex, $options: "i" } }, { email: { $regex: regex, $options: "i" } }] }).distinct("_id");
    const courseIds = await Course.find({ title: { $regex: regex, $options: "i" } }).distinct("_id");
  ...
  const [rows, total] = await Promise.all([
    Progress.find(query)
      ...
      .populate({ path: "lastWatchedVideo", select: "title" })
      .sort({ updatedAt: -1 }).skip(skip).limit(safeLimit).lean(),
    Progress.countDocuments(query),
  ]);

  return { progress: rows.map(formatProgress), pagination: { page: safePage, limit: safeLimit, total, totalPages: Math.ceil(total / safeLimit) } };
};

export const getAdminProgress = async (progressId) => {
  if (!objectId(progressId)) { const e = new Error("Invalid progress id."); e.statusCode = 400; throw e; }
  const row = await Progress.findById(progressId)
    ...
    .lean();
  if (!row) { const e = new Error("Progress record not found."); e.statusCode = 404; throw e; }
