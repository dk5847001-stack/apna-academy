        ...
        <section className="w-full max-w-[500px]">
          <div className="mb-7 flex flex-col items-center text-center">
            <div className="relative mb-5">
              ...
              <div className="relative flex h-[62px] w-[62px] items-center justify-center rounded-[20px] bg-gradient-to-br from-blue-600 to-indigo-600 text-white shadow-[0_20px_45px_-18px_rgba(37,99,235,0.65)] ring-8 ring-white"><School sx={{ fontSize: 30 }} /></div>
            </div>
          ...
          <div className="relative overflow-hidden rounded-[30px] border border-slate-200/90 bg-white/95 p-1 shadow-[0_35px_90px_-48px_rgba(15,23,42,0.45)] backdrop-blur-xl">
            ...
            <div className="rounded-[26px] border border-slate-100 bg-white px-5 py-7 sm:px-9 sm:py-9">
              <div className="mb-8">
                <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-blue-100 bg-blue-50 px-3 py-1.5 text-[11px] font-extrabold uppercase tracking-[0.08em] text-blue-700"><LockReset sx={{ fontSize: 15 }} />Account recovery</div>
              ...
              {success ? (
                <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5">
                  <div className="flex gap-3"><div className="mt-0.5 flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700">✓</div><div><p className="text-sm font-extrabold text-emerald-900">Check your email</p><p className="mt-1 text-sm leading-6 text-emerald-800">If an account exists for that email, a password reset link has been sent. The link expires in about 15 minutes.</p></div></div>
                </div>
              ) : (
                <form onSubmit={handleSubmit} noValidate className="space-y-5">
                  <TextField fullWidth label="Email address" name="email" type="email" value={email} onChange={handleChange} disabled={loading} autoComplete="email" autoFocus required InputLabelProps={{ shrink: true }} InputProps={{ startAdornment: <InputAdornment position="start"><Email className="!text-slate-400" /></InputAdornment> }} sx={{ "& .MuiInputLabel-root": { color: "#64748b", fontWeight: 600 }, "& .MuiInputLabel-root.Mui-focused": { color: "#2563eb" }, "& .MuiOutlinedInput-root": { minHeight: 58, borderRadius: "15px", backgroundColor: "#fff" }, "& .MuiOutlinedInput-notchedOutline": { borderColor: "#e2e8f0" }, "& .MuiOutlinedInput-root:hover .MuiOutlinedInput-notchedOutline": { borderColor: "#cbd5e1" }, "& .MuiOutlinedInput-root.Mui-focused .MuiOutlinedInput-notchedOutline": { borderColor: "#2563eb", borderWidth: 1.5 } }} />
                  <Button type="submit" fullWidth variant="contained" disabled={loading} endIcon={loading ? <CircularProgress size={18} className="!text-white" /> : <LockReset />} className="!min-h-[58px] !rounded-2xl !bg-gradient-to-r !from-blue-600 !to-indigo-600 !text-sm !font-extrabold !normal-case !text-white !shadow-[0_18px_35px_-18px_rgba(37,99,235,0.85)] hover:!from-blue-700 hover:!to-indigo-700 disabled:!opacity-60">{loading ? "Sending reset link..." : "Send reset link"}</Button>
                </form>
              )}

              <div className="mt-7 border-t border-slate-100 pt-6 text-center"><Link to="/login" className="inline-flex items-center gap-2 text-sm font-extrabold text-blue-600 transition-colors hover:text-blue-700 hover:underline"><ArrowBack sx={{ fontSize: 17 }} />Back to Login</Link></div>
            </div>
          </div>
        </section>
      </div>
