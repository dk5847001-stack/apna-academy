...
function AppCard({ icon: Icon, title, description, onClick, accent = "blue" }) {
  ...
  return (
    <button type="button" onClick={onClick} className="group flex w-full items-center gap-3 rounded-2xl border border-slate-700/70 bg-slate-700/30 p-3 text-left shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_10px_24px_rgba(2,6,23,0.2)] backdrop-blur-2xl backdrop-saturate-150 ring-1 ring-inset ring-white/5 transition duration-200 hover:-translate-y-0.5 hover:border-blue-400/40 hover:bg-slate-600/40">
...
export default function PremiumFooter() {
  ...
  return (
    <footer data-premium-footer="true" className="relative overflow-hidden border-t border-slate-300/25 bg-gradient-to-br from-slate-900/88 via-slate-800/88 to-slate-900/92 text-slate-200 shadow-[0_-18px_50px_rgba(15,23,42,0.24)] backdrop-blur-3xl backdrop-saturate-150 ring-1 ring-inset ring-white/10">
      ...
      <div className="pointer-events-none absolute -left-36 top-6 h-80 w-80 rounded-full bg-blue-400/22 blur-3xl" />
      <div className="pointer-events-none absolute -right-36 bottom-0 h-96 w-96 rounded-full bg-indigo-400/16 blur-3xl" />
      <div className="pointer-events-none absolute left-1/2 top-0 h-40 w-[30rem] -translate-x-1/2 rounded-full bg-cyan-300/8 blur-3xl" />
      <div className="pointer-events-none absolute inset-0 bg-gradient-to-b from-white/[0.04] via-transparent to-slate-950/10" />
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1.6fr_1fr_1fr_1.25fr]">
          <div className="min-w-0">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <button type="button" onClick={() => goTo(isLoggedIn ? DASHBOARD_URL : "/register")} className="inline-flex items-center gap-2 rounded-xl border border-slate-600 bg-slate-700/40 px-4 py-2.5 text-sm font-extrabold text-slate-200 transition hover:border-blue-400/50 hover:bg-slate-600/45 hover:text-blue-300"><Dashboard fontSize="small" />{isLoggedIn ? "Open Dashboard" : "Get Started"}</button>
            </div>
            <div className="mt-7">
            ...
            <div className="mt-7 max-w-md rounded-2xl border border-slate-700/70 bg-slate-700/30 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_12px_30px_rgba(2,6,23,0.18)] backdrop-blur-2xl backdrop-saturate-150 ring-1 ring-inset ring-white/5">
              ...
              <form onSubmit={handleSubscribe} className="mt-3 flex flex-col gap-2 sm:flex-row">
                <input value={email} onChange={(event) => setEmail(event.target.value)} required type="email" maxLength={254} placeholder="Enter your email address" className="min-w-0 flex-1 rounded-xl border border-slate-700 bg-slate-700/40 px-3.5 py-2.5 text-sm font-semibold text-white outline-none placeholder:text-slate-500 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20" />
