...
import { CheckCircleOutline, OpenInNew, Refresh, Search, VerifiedOutlined, CancelOutlined, DownloadOutlined } from "@mui/icons-material";
...
export default function Certificates() {
  ...
  const summary = useMemo(() => {
    const valid = items.filter((item) => item.isValid).length;
    return { visible: items.length, valid, invalid: items.length - valid, total: pagination.total };
  ...
  const toggleValidity = async (item) => {
    const nextState = !item.isValid;
    const action = nextState ? "validate" : "invalidate";
    if (!window.confirm(`Are you sure you want to ${action} this certificate?`)) return;
    try {
      setSavingId(item.id); setError("");
      const updated = await updateAdminCertificate(item.id, nextState);
  ...
  const exportCsv = () => {
    ...
    const headers = ["Certificate ID", "Recipient", "Student", "Email", "Course", "Issue Date", "Status", "Certificate URL", "Verification URL"];
    const rows = items.map((item) => [item.certificateId, item.recipientName, item.user?.name, item.user?.email, item.course?.title, formatDate(item.issueDate), item.isValid ? "Valid" : "Invalid", item.certificateUrl, item.verificationUrl]);
    const csv = [headers, ...rows].map((row) => row.map(csvEscape).join(",")).join("\n");
    const blob = new Blob(["\ufeff", csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `apnaacademy-certificates-page-${pagination.page}.csv`;
  ...
  return <Box>
    <Box className="mb-7"><Typography variant="h4" className="font-extrabold tracking-tight text-slate-950">Certificates</Typography><Typography className="mt-1 text-slate-500">Review issued certificates, inspect verification data, and control their public validity status.</Typography></Box>
    {error && <Alert severity="error" className="mb-5" onClose={() => setError("")}>{error}</Alert>}
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-5">
      <Metric label="Total" value={summary.total} /><Metric label="Visible" value={summary.visible} /><Metric label="Valid" value={summary.valid} /><Metric label="Invalid" value={summary.invalid} />
    </div>
    <Paper elevation={0} className="rounded-2xl border border-slate-200 p-4 sm:p-5 mb-5">
      <div className="grid grid-cols-1 md:grid-cols-[1fr_190px_auto_auto] gap-3">
        ...
        <Button variant="outlined" startIcon={<DownloadOutlined />} onClick={exportCsv} disabled={!items.length} className="rounded-xl normal-case">Export CSV</Button>
        <Button variant="outlined" startIcon={<Refresh />} onClick={() => load(pagination.page)} className="rounded-xl normal-case">Refresh</Button>
      </div>
      <Typography variant="caption" className="text-slate-500 block mt-3">CSV exports the currently loaded page and respects the active search and validity filters.</Typography>
    </Paper>
...
function Metric({ label, value }) { return <Paper elevation={0} className="rounded-2xl border border-slate-200 p-4"><Typography variant="caption" className="text-slate-500">{label}</Typography><Typography variant="h5" className="font-extrabold text-slate-950 mt-1">{value}</Typography></Paper>; }
