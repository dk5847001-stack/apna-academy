...
import { Refresh, Search, ShieldOutlined } from "@mui/icons-material";
...
export default function Users({ admin }) {
  ...
  const change = async (field, value) => {
    if (!selected || saving || selected.id === admin?.id) return;
    try {
      setSaving(true);
      const updated = await updateAdminUser(selected.id, { [field]: value });
      setSelected(updated); setUsers((items) => items.map((item) => item.id === updated.id ? updated : item));
    } catch (err) { setError(getApiErrorMessage(err, "Unable to update user.")); }
    finally { setSaving(false); }
  };

  return <Box>
    <Box className="mb-7"><Typography variant="h4" className="font-extrabold tracking-tight text-slate-950">Users</Typography><Typography className="mt-1 text-slate-500">Manage platform accounts, roles and account status.</Typography></Box>
    ...
    <Dialog open={open} onClose={() => !saving && setOpen(false)} fullWidth maxWidth="sm"><DialogTitle className="font-extrabold">Manage User</DialogTitle><DialogContent dividers>{selected && <Stack spacing={3}><Stack direction="row" spacing={2} alignItems="center"><Avatar src={selected.avatar} sx={{ width: 56, height: 56 }} className="bg-blue-50 text-blue-700">{selected.name?.[0]}</Avatar><Box><Typography variant="h6" className="font-bold">{selected.name}</Typography><Typography className="text-slate-500">{selected.email}</Typography></Box></Stack><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><FormControl fullWidth><InputLabel>Role</InputLabel><Select label="Role" value={selected.role} disabled={saving || selected.id === admin?.id} onChange={(e) => change("role", e.target.value)}><MenuItem value="user">User</MenuItem><MenuItem value="admin">Admin</MenuItem></Select></FormControl><FormControl fullWidth><InputLabel>Status</InputLabel><Select label="Status" value={selected.status} disabled={saving || selected.id === admin?.id} onChange={(e) => change("status", e.target.value)}><MenuItem value="active">Active</MenuItem><MenuItem value="inactive">Inactive</MenuItem><MenuItem value="suspended">Suspended</MenuItem></Select></FormControl></div><div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><Info label="Phone" value={selected.phone || "—"}/><Info label="Verification" value={selected.isEmailVerified ? "Verified" : "Not verified"}/><Info label="Created" value={selected.createdAt ? new Date(selected.createdAt).toLocaleString() : "—"}/><Info label="Last login" value={selected.lastLoginAt ? new Date(selected.lastLoginAt).toLocaleString() : "Never"}/></div>{selected.id === admin?.id && <Alert severity="info">Your own admin role and status cannot be changed here.</Alert>}</Stack>}</DialogContent><DialogActions><Button onClick={() => setOpen(false)} disabled={saving}>Close</Button></DialogActions></Dialog>
  </Box>;
}
